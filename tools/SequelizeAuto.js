...
function AutoSequelize(database, username, password, options) {
  if (options && options.dialect === "sqlite" && !options.storage) options.storage = database;
      ...
      function(table, _callback) {
        ...
        _.each(fields, function(field, i) {
          ...
          _.each(fieldAttr, function(attr, x) {
            var isSerialKey =
              self.tables[table][field].foreignKey && _.isFunction(self.dialect.isSerialKey) && self.dialect.isSerialKey(self.tables[table][field].foreignKey);

            // We don't need the special attribute from postgresql describe table..
            if (attr === "special" || attr === "autoIncrement") {
              return true;
            }
            if (attr === "foreignKey") {
              if (isSerialKey) {
              ...
              if (
                ...
                (!_.has(self.tables[table][field], "foreignKey") ||
                  (_.has(self.tables[table][field], "foreignKey") && !!self.tables[table][field].foreignKey.isPrimaryKey))
              )
              ...
              if (defaultVal === null || defaultVal === undefined) {
                ...
                val_text =
                  _.isString(val_text) && !val_text.match(/^sequelize\.[^(]+\(.*\)$/)
                    ? SqlString.escape(_.trim(val_text, '"'), null, self.options.dialect)
                    : val_text;
